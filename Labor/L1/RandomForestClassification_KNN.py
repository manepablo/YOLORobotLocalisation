...
for i in range(1,n_md):
    ...
    result_errors_rfc.append(meanerror)
    ...
    [sqrerror, meanerror] = metric(y_test, result)
    result_errors_knn.append(meanerror)

plt.plot(list(np.arange(1,n_md,1)),result_errors_rfc, color = 'r')
plt.plot(list(np.arange(1,n_md,1)),result_errors_knn, color = 'b')